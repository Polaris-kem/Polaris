...
(function () {
    ...
    function buildHTML() {
        return [
            ...
            '      <div><label class="form-label" style="font-size:11px;">工事番号 <span style="color:#ef4444">*</span></label>',
            '        <input id="bom-constructno" type="text" class="form-input" style="width:120px;" placeholder="例: 1001"></div>',
            '      <div><label class="form-label" style="font-size:11px;">受注登録日（KeyDate）</label>',
            '        <input id="bom-keydate" type="date" class="form-input" style="width:150px;"></div>',
            '      <div><label class="form-label" style="font-size:11px;">データ開始行</label>',
            '        <input id="bom-start-row" type="number" class="form-input" style="width:70px;" value="' + _headerStartRow + '" min="1" max="50" onchange="window._bomChangeStartRow(parseInt(this.value))"></div>',
            '    </div>',
            '    <div id="bom-drop-area" style="display:flex;flex-direction:column;align-items:center;justify-content:center;border:2px dashed #d1d5db;border-radius:10px;padding:14px 24px;cursor:pointer;background:#fafafa;min-width:200px;">',
            '      <i class="fas fa-file-excel" style="font-size:28px;color:#22c55e;margin-bottom:6px;"></i>',
            '      <span style="font-size:13px;font-weight:600;color:#374151;">クリック / ドロップ</span>',
            '      <span style="font-size:11px;color:#6b7280;margin-top:2px;">.xlsx / .xls</span>',
            '      <div id="bom-file-name" style="margin-top:4px;font-size:11px;color:#3b82f6;font-weight:500;max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"></div>',
